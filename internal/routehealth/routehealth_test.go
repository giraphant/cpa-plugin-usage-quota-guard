...
func TestGeneric429FallbackAndClamp(t *testing.T) {
	now := time.Date(2026, 7, 6, 0, 0, 0, 0, time.UTC)
	cfg := config.Default()
	cfg.RouteHealth.Rules = []config.RouteHealthRule{{Name: "generic", StatusCodes: []int{429}, DurationStrategy: "retry_after_header", FallbackDuration: config.Duration{Duration: 2 * time.Hour}, MaxDuration: config.Duration{Duration: time.Hour}}}
