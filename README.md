...
```bash
go test ./...
go build -buildmode=c-shared -o usage-quota-guard-v0.1.2.dylib ./cmd/plugin
```

On Linux, build a `.so` instead:

```bash
go build -buildmode=c-shared -o usage-quota-guard-v0.1.2.so ./cmd/plugin
```

CPA plugin filenames should follow CPA's versioned convention, for example `usage-quota-guard-v0.1.2.dylib`.
...
```text
usage-quota-guard_0.1.2_linux_amd64.zip
```
